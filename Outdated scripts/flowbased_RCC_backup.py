...
# Create space to save ptdf results (calculated below)
if not os.path.exists('./PTDF results/with cont'):
    os.mkdir('./PTDF results/with cont')
...
for hour in range(1,25):
    ...
    ptdf.isContSens = 1             # Select consider contingencies
    ...
    res.f_name=rf'C:\Users\alice\OneDrive - Lund University\Dokument\Doktorand IEA\Kurser\Flowbased\Python - Flowbased\PTDF results/with cont/ptdf_{hour}.csv'
    res.ExportFullRange()
...
for hour in range(1,25):
    ...
    test = ptdf_res[[el for el in ptdf_res.columns if any(col in el for col in CNEC.columns)]]
